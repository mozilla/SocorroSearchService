...
public class NodeClientSingleton {
  ...
  private NodeClientSingleton(Properties properties) {
    ...
    System.err.println("loading this");
    String configFile = properties.getProperty("es.config.path", "conf/elasticsearch.yml");

    this.node = nodeBuilder().loadConfigSettings(false).settings(ImmutableSettings.settingsBuilder().loadFromClasspath(configFile)).node().start();
    
    this.client = node.client();
  }
  
  public NodeClientSingleton() throws IOException {
    System.err.println("loading this prop");
    ...
    InputStream input = new FileInputStream(props.getProperty("es.config.path", "conf/elasticsearch.json"));

    this.node = nodeBuilder().loadConfigSettings(false).settings(ImmutableSettings.settingsBuilder().loadFromStream("", input)).node().start();
